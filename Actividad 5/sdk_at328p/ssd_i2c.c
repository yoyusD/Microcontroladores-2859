...
void ssd_num_write(char dato, char pwn_select,int position){
	...
	if(position == 0){
		if(pwn_select==1){
			set_cursor(2,25);
			ssd_write(0x00);
			}else if(pwn_select==2){
			set_cursor(3,38);
			ssd_write(0x00);
			}else if(pwn_select==3){
			set_cursor(4,32);
			ssd_write(0x00);
			}
	}else if(position == 1){
...
void graph_volt(float voltage, char loc){ //El loc me lo define el contador que se encuentra en adc.c 
	loc = loc + 15; 
	set_cursor(4,loc);
	ssd_write(0x00);
